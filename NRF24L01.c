...
#define CE_PORT GPIOA    // Chip enable activate Rx or Tx mode
...
uint8_t NRF24_Transmit(uint8_t *data){
	...
	if((fifostatus & (1<<4)) && (!(fifostatus & (1<<3)))){
		cmdtosend = FLUSH_TX;
		nrf24_Send_Cmd(cmdtosend); // làm rỗng bộ đệm (xóa)
...
void NRF24_RxMode(uint8_t *Address, uint8_t channel){
	...
	// disable các giá trị ban đầu của các pipe khác và chọn 1 pipe

	uint8_t en_rxaddr = nrf24_Read_Register(EN_RXADDR);  // lấy giá trị của thanh ghi EN_RXADDR
	en_rxaddr |= (1<<1); // set bit thứ nhất của thanh ghi đó lên 1 để chọn pipe 1;

	nrf24_Write_Register(EN_RXADDR, en_rxaddr); // ghi giá trị value ban đầu của pipe 1 vào thanh ghi (chọn pipe 1)


	// setup address của pipe
	nrf24_Write_Register_Multiple(RX_ADDR_P1, Address, 5);  // Ghi address cho pipe 1

	// setup data size cho data pipe (mặc định là 32 bytes và k dùng kiểu dynamic byte)
...
// Kiểm tra xem có dữ liệu ở bất cứ 1 pipe nào hay k
uint8_t isDataAvailable(int pipeNumber){
	uint8_t status = nrf24_Read_Register(STATUS);

	// Kiểm tra có dữ liệu lưu trong FIFO không hay là chuẩn bị có dữ liệu được lưu vào hay k
	// ( tức kiểm tra xem có dữ liệu ở bất cứ pipe
	// nào trong 6 pipe hay là có dữ liệu mới chuẩn bị tới, nếu có set bit 6 lên 1)
